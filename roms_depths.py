...
__author__         = "Daniel Dauhajre, Cigdem Akan, Minna Ho"
__date__           = "April 2026"
__email__          = "ddauhajre@atmos.ucla.edu, minnaho@atmos.ucla.edu"
__python_version__ = "3.7.8"
...
def get_zw_loop(nc_roms, nc_grd, tind=None):
    """
    GET DEPTHS reading zeta directly from nc_roms for W-points (z_w)
    for all or select time steps in given ROMS output nc_roms
    ...
    # Get N from dimensions (N is the number of rho-levels; set_depth handles the N+1 for w-levels internally)
    if 's_rho' in nc_roms.dimensions:
        N = len(nc_roms.dimensions['s_rho'])
    else:
        N = nc_roms.variables['s_rho'].shape[0]
    ...
    # Set Vertical Transformation params
    Vtrans = 2
    Vstret = 4
    if 'Vtransform' in nc_roms.variables:
        Vtrans = nc_roms.variables['Vtransform'][:]
    ...
    if tind is not None:
        ...
        # Grid type '5' is used for W-points
    ...
    # CASE B: All Time Steps (4D Output)
        ...
        print(f"Calculating W-depths for {Nt} time steps using set_depth...")
        
        # Process first step to determine output shape (this naturally captures N+1)
        z_0 = set_depth(Vtrans, Vstret, theta_s, theta_b, hc, N, 5, h, zeta_all[0,:,:]).T
        z_0 = np.swapaxes(z_0, 1, 2)
        
        # Allocate 4D array dynamically based on z_0 shape [N+1, X, Y]
        z_4d = np.zeros((Nt, *z_0.shape))
        z_4d[0,:,:,:] = z_0
        ...
        for t in range(1, Nt):
            print(str(t) + ' of ' + str(Nt))
            z_t = set_depth(Vtrans, Vstret, theta_s, theta_b, hc, N, 5, h, zeta_all[t,:,:]).T
            z_4d[t,:,:,:] = np.swapaxes(z_t, 1, 2)
    ...
    ##########################################################
	######################################
	# BELOW: FUNCTIONS BY CIGDEM AKAN
