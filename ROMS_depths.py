...
def get_zr_L2grid(nc_grd):
    ...
    #BOTTOM TOPOGRAPHY
    h = np.array(nc_grd.variables['h'])

    zeta = np.zeros((Ly_all,Lx_all))
    z_r = set_depth(Vtrans, Vstret, theta_s, theta_b, hc, N, 1, h, zeta).T
...
def get_zr_tind(nc_roms, nc_grd, tind, dim_bounds):
    ...
    zeta = nc_roms.variables['zeta'][tind,eta_0:eta_1,xi_0:xi_1]
    #print('Calculating z_r at time-step =', tind)
    z_r = set_depth(Vtrans, Vstret, theta_s, theta_b, hc, N, 1, h, zeta).T
    #print('z_r done')
...
def get_zs3d(nc_roms,nc_grd,dim_bounds,varname,igrid=1):
    ...
    #DECLARE DEPTH ARRAY
    ######################
    N = len(nc_roms.variables[varname][:,0,0])
    [Ly_all,Lx_all] = nc_roms.variables[varname][0,:,:].shape
    [eta_0, eta_1, xi_0, xi_1] = dim_bounds
    Ly = eta_1 - eta_0
    Lx = xi_1 - xi_0
    ...
    zeta = nc_roms.variables['zeta'][eta_0:eta_1,xi_0:xi_1]
    z_r = set_depth(Vtrans, Vstret, theta_s, theta_b, hc, N, 1, h, zeta).T
    #print('z_r done')
